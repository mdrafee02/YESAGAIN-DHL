...
def detect_product_profile(product_name):
    """
    Pick the correct customs/packaging profile from the order's product
    name. Defaults to "laptop" for any name that doesn't match a known
    keyword — preserves current behaviour for every order type already
    shipping correctly.
    """
...
def transform(df, ya_rma_map=None, crs_rma_map=None, price_map=None):
    ...
    for _, r in df.iterrows():
        ...
        pp = detect_product_profile(product_name)
...
def book_return_label(original_awb):
    ...
    pp = detect_product_profile(_str(
        row.get("Sales Product Name (from Sales Order Lines)") or
