...
# ── List log files ───────────────────────────────────────────────────
# Import book_return_label at module level so errors are visible in logs
try:
    from airtable_transform import book_return_label as _book_return_label
    _return_label_import_error = None
except Exception as _e:
    _book_return_label = None
...
def book_return():
    ...
    if _return_label_import_error:
        return jsonify({"success": False, "error": f"Import error: {_return_label_import_error}"}), 500
    ...
    try:
        ...
        print(f"[book-return] Booking return label for AWB: {awb}")
        result = _book_return_label(awb)
